...
Runs local MongoDB by default, and points at MongoDB Atlas + Render with one environment variable when I want it hosted.
...
<a href="https://kambaz-next-js-vicky-a6.vercel.app/Account/Signin"><b>Frontend it serves</b></a> · https://kambaz-next-js-vicky-a6.vercel.app/Account/Signin
...
By default it connects to a local MongoDB. Swap the connection string for an Atlas URI and set `SERVER_ENV=production`, and the same code runs on Render behind secure cross-site cookies.
...
Your data is still there — it lives in the Mongo volume, not the server process.
